...
def recreate_server [--server: record] {
    ...
    let ipv4_ptr_response = (
        http post -H (default_headers) -t application/json $'https://api.hetzner.cloud/v1/primary_ips/($ipv4.id)/actions/change_dns_ptr' {
            ip: $ipv4.ip,
            dns_ptr: $fqdn,
        } | await_action
    )
    let ipv6_ptr_response = (
        http post -H (default_headers) -t application/json $'https://api.hetzner.cloud/v1/primary_ips/($ipv6.id)/actions/change_dns_ptr' {
